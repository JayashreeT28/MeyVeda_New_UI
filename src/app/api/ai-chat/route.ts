...
export async function GET() {
  let status = await getFlaskStatus();
  
  if (!status) {
    // Start Python server in background if it's down
    startPythonServer();
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "load") {
      ...
      if (!status) {
        startPythonServer();
        // Give the server a small moment to launch (asynchronously), return starting state
        return NextResponse.json({
          loaded: false,
          message: "Starting Python backend service...",
          progress: 5,
